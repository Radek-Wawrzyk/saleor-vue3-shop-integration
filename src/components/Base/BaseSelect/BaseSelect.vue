<template>
  <div
    class="base-select"
    :class="[error ? 'base-select--error' : false, disabled ? 'base-select--disabled' : false]"
    ref="selectRef"
  >
    <button
    ...
    >
      ...
      <svg
        ...
        :class="[isActive ? 'base-select__inner-icon--is-active' : false]"
        aria-hidden="true"
        focusable="false"
        viewBox="0 0 24 24"
        role="img"
    ...
    <transition name="fade">
      <div class="base-select__error" aria-label="error" v-if="error">
        {{ error }}
      </div>
    </transition>

    <transition name="fade" mode="out-in">
      <ul class="base-select-list" v-if="isActive">
        <li class="base-select-list__option" v-for="option in options" :key="option.value">
          <button
